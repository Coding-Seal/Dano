...
def create_plot(data):
    ...
    for mcc in data.code.unique():
        ...
        if np.size(x_offline):
            ...
            f_offline = Akima(offline["day_time"].replace(day).values, y_offline)
        if np.size(x_online):
            f_online = Akima(online["day_time"].replace(day).values, y_online)
            model_online = LinearRegression().fit(x_online, y_online)

        fig = go.Figure()
        # offline graph
        if np.size(x_offline):
            fig.add_trace(go.Scatter(x=np.arange(1,29), y=model_offline.predict(key), name="offline_trend",
                                     mode='lines+markers',
                                     marker=dict(color='LightSkyBlue')))
            fig.add_trace(go.Scatter(x=np.arange(1, 29, 10**-2), y=f_offline(np.arange(1, 29,10**-2)), name="offline_akima",
                                     mode='lines',
                                     marker=dict(color='LightSkyBlue')))
            fig.add_trace(go.Scatter(x=offline["day_time"].replace(day), y=offline["transaction_amt"], mode='markers', name='',
                                     marker=dict(color='LightSkyBlue', size=10,
                                                 line=dict(color='MediumPurple', width=3))))
        # online graph
        if np.size(x_online):
            fig.add_trace(go.Scatter(x=np.arange(1,29), y=model_online.predict(key), name="online_trend",
                                     mode='lines+markers',
                                     marker=dict(color='Red')))
            fig.add_trace(
                go.Scatter(x=np.arange(1, 29, 10 ** -2), y=f_online(np.arange(1, 29, 10 ** -2)), name="online_akima",
                           mode='lines',
                           marker=dict(color='Red')))
            fig.add_trace(go.Scatter(x=online["day_time"].replace(day), y=online["transaction_amt"], mode='markers', name='',
                                     marker=dict(color='Red', size=5, line=dict(color='MediumPurple', width=3))))
